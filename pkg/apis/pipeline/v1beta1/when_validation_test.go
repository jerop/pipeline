	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.wes.validate(); err != nil {
				t.Errorf("When.validate() returned an error for valid when expressions: %s", tt.wes)
			}
		})
	}
}
...
func TestWhenExpressions_Invalid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.wes.validate(); err == nil {
				t.Errorf("When.validate() did not return error for invalid when expressions: %s, %s", tt.wes, err)
			}
		})
	}
}
